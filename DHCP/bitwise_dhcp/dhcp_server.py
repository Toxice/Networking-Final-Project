    ...
    def serve(self, port=SERVER_PORT):
        ...
        if sys.platform.startswith('linux'):
            try:
                ...
                print(f"[DHCP] Linux: Bound to interface {self.interface}")
            except PermissionError:
                print("[DHCP] Permission denied: DHCP servers require sudo on Linux.")
            ...
            try:
                ...
                print(f"[DHCP] Windows: Bound to IP {self.server_ip} on interface {self.interface}")
            except Exception as e:
                print(f"[DHCP] Windows Bind Error: {e}")
        ...
        print(f"[DHCP] Server active. IP Serving pool: {self.ip_pool[0]}-{self.ip_pool[-1]}")
        while True:
...
if __name__ == "__main__":
    ...
    print(f"[DHCP] Using Base IP: {args.ip}")  # Confirms what was chosen
